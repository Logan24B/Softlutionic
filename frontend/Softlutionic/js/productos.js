...
function renderServices() {
  ...
  const average = serviceState.services.length
    ? serviceState.services.reduce((sum, service) => sum + Number(service.Precio || 0), 0) / serviceState.services.length
    : 0;

  serviceElements.total.textContent = serviceState.services.length;
  serviceElements.average.textContent = formatMoney(average);

  serviceState.services.forEach((service) => {
    const row = document.createElement("tr");
    row.append(
      createCell(`SERV-${String(service.id).padStart(3, "0")}`, { strong: true }),
      createCell(service.Nombre_Servicio, { strong: true }),
      createCell(service.Descripcion),
      createCell(service.Duracion),
      createCell(formatMoney(service.Precio), { strong: true }),
      createActionCell(service)
    );
    fragment.appendChild(row);
  });

  if (!serviceState.services.length) {
    const row = document.createElement("tr");
    const cell = document.createElement("td");
    cell.colSpan = 6;
    cell.className = "muted-cell";
    cell.textContent = "No hay servicios registrados en la base de datos.";
